...
* Added the SQL functions `isiri`/`isuri`, `iri`/`uri`,`datatype(text)`,`str(text)`, `strdt(text, text)`, `langmatches(text,text)`, `strlang(text, text)`, `encode_for_uri(text)`, `contains(text, text)`, `strstarts(text, text)`, `strends(text, text)`. `strbefore(text, text)` and `strafter(text, text)`, which replicate the behavior of SPARQL’s `isIRI`/`isURI`, `IRI`/`URI`,`DATATYPE`,`STR`,`STRDT`,`LANGMATCHES`, `STRLANG`, `ENCODE_FOR_URI`, `CONTAINS`, `STRSTARTS`,`STRENDS`,`STRBEFORE`, and `STRAFTER`. These functions are translated to their SPARQL equivalents when pushed down to the foreign data wrapper.
