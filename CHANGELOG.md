...
* **A clone kept reading a table the caller had lost the right to read**: `rdf_fdw_clone_table()` checks that the caller may select from the foreign table, and then commits each page of the clone as it goes — `commit_page` defaults to `true`. A commit ends the transaction, and with it both the lock taken on the foreign table when its name was resolved and the moment the privilege check was made against. Every page after the first was read with neither: a `REVOKE` committed by another session in the meantime went unnoticed, and the clone carried on reading and inserting rows until it reached the end. The lock and the check are now taken again for each page, so a clone stops at the first page it is no longer entitled to.
...
* **A malformed IRI or blank node term could change the SPARQL built from it**: `isIRI()` accepted any `<...>` whose body was free of quotes and ASCII whitespace, and `isBlank()` accepted any bytes at all after the `_:`. The SPARQL grammar is far narrower — rule [139] also excludes `<`, `>`, `{`, `}`, `|`, `^`, `` ` ``, `\` and the ASCII controls from an IRI, and rule [142] restricts a blank node label to `PN_CHARS`. Since the deparser and the SPARQL Update builder copy such a term into the generated query verbatim, trusting it to be one token, a body carrying a `>` ended the term early and left the bytes after it to be read as further tokens: a pushed-down `FILTER` could then express something other than the `WHERE` it came from, and an `INSERT`/`DELETE` could carry triples the statement did not name. Both classifiers now enforce the grammar, and a term that fails is handled as a plain string literal and quoted, which is what a term containing a space already did. The `IRI()` and `URI()` constructors, and `STRDT()` through them, built their result without checking it at all; because they are asked for an IRI rather than merely offered one, an invalid body is now a type error there, as SPARQL 1.1 §17.4.2.8 specifies. Terms read back from an endpoint are covered as well, since the scan converts them with the type's input function.

### RDF values, literals and functions

* **A literal was treated as a number without its lexical form being checked against its datatype**: `isNumeric()` decided the question by handing the lexical form to `strtod()` and asking only whether the whole string was consumed. `strtod()` accepts spellings no XSD numeric datatype has — hexadecimal, `nan`, `inf` — and it knows nothing about the datatype the literal actually carries, so `"0x10"^^xsd:integer` was read as the number 16, `"1.5"^^xsd:integer` as 1.5, and `"nan"^^xsd:integer` as a number. The integer subtypes were never range-checked either, although each has its own value space: `"1200"^^xsd:byte`, `"99999"^^xsd:short`, `"-5"^^xsd:nonNegativeInteger`, `"0"^^xsd:positiveInteger` and `"-1"^^xsd:unsignedByte` were all numeric. Every one of these is an ill-typed literal that SPARQL gives no value, and Apache Jena and Virtuoso report all of them as non-numeric. The lexical form is now validated against the datatype's own lexical space and, for the integer subtypes, against its value range; `xsd:decimal` admits neither an exponent nor `INF`/`NaN`, which is why casting `"INF"^^xsd:decimal` to `numeric` now fails rather than yielding `Infinity`. Surrounding whitespace stays acceptable, since every XSD numeric datatype fixes `whiteSpace="collapse"` and `"  12"^^xsd:integer` is a valid integer. Literals a query filtered with a pushed-down `isNumeric()` are decided by the endpoint, so a store that reads these differently can still admit rows whose values do not exist.

* **`sparql.describe()` lost the statements of a blank node the document did not name**: An RDF/XML node element carrying neither `rdf:about` nor `rdf:nodeID` describes a blank node, and the reader is asked to generate an identifier for it. Written at the top level, such an element was skipped whole and everything it said went missing. Written inside a property element — the object spelled out in place rather than referred to by `rdf:resource` or `rdf:nodeID` — it was read as character data instead, so a described node came back as a literal made of its own property values: a node with `name` "Alice" and `age` "42" arrived as the literal `"Alice42"`, and its two statements were lost with it. Both now report the node under a generated label, and the statements it makes are reported too. The labels are numeric, which an `rdf:nodeID` may not be, so a generated one cannot collide with a label the document wrote.

* **A language tag was half normalised**: A tag was lowercased up to its first hyphen and left as written after it, so one tag reached storage as several terms — `@EN-GB` as `en-GB`, `@en-gb` as `en-gb`, `@ZH-Hant-TW` as `zh-Hant-TW`. Value equality called those equal, the operator class compared the stored term and did not, and `sparql.lang()` gave two strings for one tag. RDF 1.1 Concepts §3.3 settles it: "Lexical representations of language tags MAY be converted to lower case. The value space of language tags is always in lower case." The whole tag is now lowercased, which makes the three agree.

  Terms already stored keep the tag they were stored with, since an extension upgrade does not rewrite table data. A column holding both spellings will see them as two terms until the rows are written again. Nothing else changes: a tag was already matched without regard to case when two terms were compared, and both Fuseki and GraphDB match a lowercased tag against one stored in mixed case, in a `FILTER` and in a triple pattern alike, so a condition sent to an endpoint still selects the same rows.
...
* **A foreign table with no columns counted no rows**: A scan of a foreign table that has no columns returned nothing at all, because it gave up before making the request. The request is worth making: `count(*)` is about the only question such a table can be asked, and the endpoint still answers it. The generated SPARQL was already valid — the projection becomes `SELECT *` — so the records came back and were discarded, and `count(*)` reported `0` for a table the endpoint had just returned rows for.
