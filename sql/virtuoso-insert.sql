 ...
 * graph in virtuoso. This exercises the cross-FDW INSERT-SELECT code path
...
CREATE FOREIGN TABLE rdbms_virtuoso (
  s rdfnode OPTIONS (variable '?s'),
  p rdfnode OPTIONS (variable '?p'),
  o rdfnode OPTIONS (variable '?o')
)
...
INSERT INTO rdbms_virtuoso (s, p, o)
SELECT s, p, o FROM rdbms_wikidata;

SELECT * FROM rdbms_virtuoso 
...
ALTER SERVER virtuoso OPTIONS (ADD update_url 'foo');
INSERT INTO ft (subject, predicate, object) VALUES
('<https://www.uni-muenster.de>', '<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>', 'http://dbpedia.org/resource/University');

/* invalid triple pattern - no sparql_update_pattern OPTION */
ALTER SERVER virtuoso OPTIONS (DROP update_url);
...
ALTER FOREIGN TABLE ft OPTIONS (SET sparql_update_pattern 'GRAPH <http://rdf-fdw.test/default> { ?s ?p ?o . }');
INSERT INTO ft (subject, predicate, object) VALUES
...
SERVER virtuoso OPTIONS (SET user 'dba', SET password 'foo'); -- wrong password

INSERT INTO ft (subject, predicate, object) VALUES
('<https://www.uni-muenster.de>', '<http://www.w3.org/2000/01/rdf-schema#label>', '"foo"@de');

ALTER USER MAPPING FOR postgres
SERVER virtuoso OPTIONS (SET user 'dba', SET password 'secret'); -- restore correct password

/* read-only server: blocks INSERT regardless of triple pattern */
ALTER SERVER virtuoso OPTIONS (ADD readonly 'true');
INSERT INTO ft (subject, predicate, object) VALUES
('<https://www.uni-muenster.de>', '<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>', 'http://dbpedia.org/resource/University');
...
DELETE FROM ft;
DELETE FROM rdbms_virtuoso;    -- clear <http://www.uni-muenster.de/graph>
